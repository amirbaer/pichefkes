...
impl From<&str> for Person {
    fn from(s: &str) -> Self {
        ...
        let parts_count = split_iterator.clone().count();

        if parts_count != 2 {
            return Person::default()
        }
        
        let name = String::from(split_iterator.next().unwrap());
        if name.is_empty() {
            return Person::default()
        }

        let age_result = String::from(split_iterator.next().unwrap()).parse::<u8>();
